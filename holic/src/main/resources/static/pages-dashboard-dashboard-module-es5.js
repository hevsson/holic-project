...
(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["pages-dashboard-dashboard-module"], {
  ...
  function srcAppPagesDashboardDashboardComponentTs(module, __webpack_exports__, __webpack_require__) {
    ...
    /*#__PURE__*/
    function () {
      function DashboardComponent(router, activatedRoute, userProfileService, snackBar, deviceService) {
      ...
      _createClass(DashboardComponent, [{
        ...
        value: function ngAfterViewInit() {
          ...
          this.main.addEventListener('playing', function () {
            if (!_this.interviewVideo.mute && !_this.reportVideo.mute && !_this.lifestyleVideo.mute && !_this.musicVideo.mute && !_this.sportVideo.mute && !_this.interview2Video.mute) {
              _this.interviewVideo.mute = true;
              _this.reportVideo.mute = true;
              _this.lifestyleVideo.mute = true;
              _this.musicVideo.mute = true;
              _this.sportVideo.mute = true;
              _this.interview2Video.mute = true;
              _this.mainVideo.mute = false;
            }
          });
          ...
          this.main.addEventListener('loadedmetadata', function () {
            ...
            _this.mainVideo.time = x % _this.main.duration;
          ...
          this.report.addEventListener('loadedmetadata', function () {
            var datebb = new Date();
            var x = datebb.valueOf() / 1000;
            _this.reportVideo.time = x % _this.report.duration;
            _this.report.currentTime = x % _this.report.duration;
          });
          ...
          this.sport.addEventListener('loadedmetadata', function () {
            ...
            _this.sportVideo.time = x % _this.sport.duration;
            _this.sport.currentTime = x % _this.sport.duration;
          });
          ...
          this.music.addEventListener('loadedmetadata', function () {
            var datebb = new Date();
            var x = datebb.valueOf() / 1000;
            _this.musicVideo.time = x % _this.music.duration;
            _this.music.currentTime = x % _this.music.duration;
          });
          ...
          this.lifestyle.addEventListener('loadedmetadata', function () {
            var datebb = new Date();
            var x = datebb.valueOf() / 1000;
            _this.lifestyleVideo.time = x % _this.lifestyle.duration;
            _this.lifestyle.currentTime = x % _this.lifestyle.duration;
          });
          ...
          this.interview2.addEventListener('loadedmetadata', function () {
            var datebb = new Date();
            var x = datebb.valueOf() / 1000;
            _this.interview2Video.time = x % _this.interview2.duration;
            _this.interview2.currentTime = x % _this.interview2.duration;
          });
          this.interview = this.matinterviewVideo.getVideoTag();
          this.interview.addEventListener('mouseover', function () {
            _this.playInterview();
          });
          this.interview.addEventListener('mouseout', function () {
            _this.muteInterview();
          });
          this.interview.addEventListener('loadedmetadata', function () {
            var datebb = new Date();
            var x = datebb.valueOf() / 1000;
            _this.interviewVideo.time = x % _this.interview.duration;
            _this.interview.currentTime = x % _this.interview.duration;
          });
        }
      }, {
        key: "playInterview",
        ...
        value: function userExist() {
          ...
          if (this.searchtext !== undefined && this.searchtext !== '') {
            this.userProfileService.userexist(this.searchtext).subscribe(function (p) {
              if (p === false) {
                _this2.snackBar.open('User does not exist', 'close', {
              ...
              if (p === true) {
                _this2.router.navigate(['/user/' + _this2.searchtext]);
              }
            });
          }
        }
        ...
        value: function previousUser() {
          ...
          this.userProfileService.previousUser('DASHBOARD_HOLIC').subscribe(function (p) {
            _this3.router.navigate(['/user/' + p.login]);
          }, function (error) {
        ...
        value: function nextUser() {
          ...
          this.userProfileService.nextUser('DASHBOARD_HOLIC').subscribe(function (p) {
